...
public struct Parser {
    ...
    // Format: "container_path:host_path" (e.g., "/var/run/docker.sock:/tmp/docker.sock")
    ...
    // Format: "container_path:host_path" -> PublishSocket
    private static func publishSocket(_ socket: String) throws -> PublishSocket {
        // Split by colon to two parts: [container_path, host_path]
        let parts = socket.split(separator: ":")

        switch parts.count {
        case 2:
            // Extract container and host paths
            let containerPath = String(parts[0])
            let hostPath = String(parts[1])
        ...
        default:
            throw ContainerizationError(
                .invalidArgument,
                message:
                    "invalid publish-socket format \(socket). Expected: container_path:host_path")
        }
    }
}
